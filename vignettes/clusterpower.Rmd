...
3. For each cluster $i$:
  a. Sample, from a normal distribution, residual error $e_{ij}$.
  b. Calculate $Y_{ij}=\beta_{0}+\beta_{1}I(G_{ij}=1)+...+b_{i}+e_{ij}$.
4. Perform data analysis, record whether null hypothesis was rejected.
5. Repeat previous steps $K$ times.
\end(list)
Power is estimated by $\sum r_{k}/K$, where $r_{k}=1$ is the null was rejected in iteration $k$. The confidence interval (CI) for the simulated power approximation can be calculated based on the properties of the binomial distribution. The approach can be modified to accomidate poisson or dichotomous outcomes, and to adjust for differing cluster sizes. For more details, see @gatsonis2017methods.
  
# ICC and variance approaches
  The equation for the ICC is $\sigma_{b}^2/(\sigma_{b}^2+\sigma^2), where... (do we need this much detail?)
...
\begin{equation}
x_{itj}= \beta_{0}+x_{ij}\beta_{1}+b_{i}+e_{ij}
\end{equation}
...
Frequently investigators need to design three-arm trials, or four-arm trials—the latter case is especially common in the case of factorial designs used to assess potential interaction or synergy between two treatments.  
