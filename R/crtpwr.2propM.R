...
crtpwr.2propM <- function(alpha = 0.05, power = 0.80,
                         nclusters = NA, nsubjects = NA, p1 = NA, p2 = NA,
                         cvm = 0, p1inc = TRUE,
  ...
  needlist <- list(alpha, power, nclusters, nsubjects, p1, p2, cvm)
  neednames <- c("alpha", "power", "nclusters", "nsubjects", "p1", "p2", "cvm")
  needind <- which(unlist(lapply(needlist, is.na))) # find NA index
  
  if (length(needind) != 1) {
    stop("Exactly one of 'alpha', 'power', 'nclusters', 'nsubjects', 'p1', 'p2', or 'cvm' must be NA.")
  }
  ...
  pwr <- quote({
    ...
    sdd <- sqrt((p1*(1-p1) + p2*(1-p2))/nsubjects + cvm^2*(p1^2 + p2^2))
    pnorm(sqrt((nclusters - 2)*(p1 -p2)^2)/sdd - zcrit, lower.tail = TRUE)# +
