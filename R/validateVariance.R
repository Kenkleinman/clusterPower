...
validateVariance <-
  function(dist = NULL,
    ...
    if (dist == "norm") {
      ...
      if (length(difference) == 1) {
        ...
        if (sum(parm1.args) == 0 &&
          ...
          stop(
            "At least one of the following terms has been misspecified: ICC, sigma_sq, sigma_b_sq"
          )
        }
      }
    ...
    if (!is.logical(poor.fit.override)) {
      stop(
        "poor.fit.override must be either FALSE (Stop simulations if estimated power is <0.5 or
