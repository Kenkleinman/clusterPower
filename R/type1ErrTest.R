...
type1ErrTest <- function(sigma_sq_, sigma_b_sq_, nsubjects_){
  ...
  for (i in 1:length(nobs)) {
    for (j in 1:length(nobs[[i]])) {
      ...
      alert[j] <- as.vector(ifelse((nobstemp < 5 & ICCtemp < 0.1 & clusterstemp < 20) | 
      ...
      if (sum(alert) > 0) {
        warning("ClusterPower uses maximum likelihood inference, which depends on asymptotic results. 
